 ...
 * in main (which streams live progress and harvests the report), removes the
 * worktree, and acks so main can free a queue slot.
 */
export function useDiagnosticoDispatch(): void {
  useEffect(() => {
    const unsubscribe = window.api.diagnosticos.onDispatchRequested(async (request) => {
      ...
      try {
        // 'skip' setup: read-only investigation needs no deps, and skipping avoids
        // a node_modules junction so removal afterwards is safe.
        ...
        // DIAGNOSTICO.md, and exits. Main streams live progress into the
        // Diagnostico and harvests the report, setting status before resolving.
